...
// A = 2^16 * a1 + a0
// B = 2^16 * b1 + b0
// (2^16 * a1 + a0) * (2^16 * b1 + b0)
// = 2^32 * a1b1 + 2^16 * (a1b0 + a0b1) + a0b0
// x = 2^16 * (a1b0 + a0b1) + a0b0
// y = a1b1 + x >> 32
//   = a1b1 + (a1b0 + a0b1 + a0b0 >> 16) >> 16
//   = a1b1 + hadd(a0b0 >> 16 + a0b1, a1b0) >> 15
...
// = 2^64 * a1b1 + 2^32 * (a1b0 + a0b1) + a0b0
// = 2^128*y11 + 2^96*(y01+y10) + 2^64*(x11+y00) + 2^32*(x01+x10) + x00. assume x[i][y] = a[i]b[1].x
// r.x = x00
// r.y = y00 + x01 + x10
// r.z = x11 + y01 + y10
// r.w = y11
...
fn gl_fp_add(a: Felt, b: Felt) -> Felt {
  ...
  // If overflow occurs, additionally add 2^64 - 2^32 + 1
...
fn gl_ext_mul(a: array<Felt,3>, b: array<Felt,3>) -> array<Felt, 3> {
  let a0b0 = gl_fp_mul(a[0] , b[0]); // m₁
  let a1b1 = gl_fp_mul(a[1] , b[1]); // m₂
  let a2b2 = gl_fp_mul(a[2] , b[2]); // m₃

  let a0b0_a0b1_a1b0_a1b1 = gl_fp_mul(gl_fp_add(a[0] , a[1]) , gl_fp_add(b[0] , b[1])); // m₄
  let a0b0_a0b2_a2b0_a2b2 = gl_fp_mul(gl_fp_add(a[0] , a[2]) , gl_fp_add(b[0] , b[2])); // m₅
  let a1b1_a1b2_a2b1_a2b2 = gl_fp_mul(gl_fp_add(a[1] , a[2]) , gl_fp_add(b[1] , b[2])); // m₆

  let a0b0_minus_a1b1 = gl_fp_sub(a0b0 , a1b1); // a₀b₀ - a₁b₁

  // m₆ + a₀b₀ - a₁b₁ - a₂b₂
  //   = a₀b₀ + a₁b₂ + a₂b₁
  let a0b0_a1b2_a2b1 = gl_fp_sub(gl_fp_add(a1b1_a1b2_a2b1_a2b2 , a0b0_minus_a1b1) , a2b2);

  let t0 = gl_fp_add(a0b0_a0b1_a1b0_a1b1 , a1b1_a1b2_a2b1_a2b2); // m₄ + m₆
  let t1 = gl_fp_add(gl_fp_add(a1b1, a1b1), a0b0); // a₁b₁ + a₁b₁ + a₀b₀

  let t3 = gl_fp_sub(t0, t1); // m₄ + m₆ - (a₁b₁ + a₁b₁ + a₀b₀)

  let a0b2_a1b1_a2b0_a2b2 = gl_fp_sub(a0b0_a0b2_a2b0_a2b2 , a0b0_minus_a1b1); // m₅ - a₀b₀ + a₁b₁
  return array<Felt, 3>(a0b0_a1b2_a2b1, t3, a0b2_a1b1_a2b0_a2b2);
}

// a² = (a₀ + a₁X + a₂X²)²
//    = a₀² + 2a₀a₁X + (2a₀a₂ + a₁²)X² + 2a₁a₂X³ + a₂²X⁴
//    = a₀² + 2a₀a₁X + (2a₀a₂ + a₁²)X² + 2a₁a₂(X+1) + a₂²(X²+X)
//
// out₀ = a₀² + 2a₁a₂
// out₁ = 2a₀a₁ + 2a₁a₂ + a₂²
// out₂ = 2a₀a₂ + a₁² + a₂²
fn gl_ext_square(a0: Felt, a1: Felt, a2: Felt) -> array<Felt, 3> {
  let a2_sq = gl_fp_square(a2);  // a₂²
  let a1_a2 = gl_fp_mul(a1, a2); // a₁a₂

  let out0 = gl_fp_add(gl_fp_square(a0), gl_fp_double(a1_a2)); // a₀² + 2a₁a₂
  let out1 = gl_fp_add(gl_fp_double(gl_fp_add(gl_fp_mul(a0 , a1) , a1_a2)) , a2_sq); // 2a₀a₁ + 2a₁a₂ + a₂²
  let out2 = gl_fp_add(gl_fp_add(gl_fp_double(gl_fp_mul(a0 , a2)) , gl_fp_square(a1)) , a2_sq); // 2a₀a₂ + a₁² + a₂²
...
fn mul_mont(x: vec2<u32>, y: vec2<u32>) -> vec2<u32> {
  ...
  let xl = combo.xy; // T mod R
  let xh = combo.zw;
  let tmp = vec2<u32>(0u, xl.x);
  let a_s = add_w_overflowing(xl, tmp); // (T mod R)  * (2^32 + 1)
...
fn recombine_u32_v6(a: u32, b: u32, c: u32, d: u32) -> vec2<u32> {
    ...
    let adjust_lo = 4294967295u*(v_res_hi_0.y+v_res_hi_1.y);//select(0, 0u32.wrapping_sub(1u32), res_hi_0_carry||res_hi_1_carry);
...
fn block2(x: array<vec2<i32>, 3>, y: array<vec2<i32>, 3>) -> array<vec2<i32>, 3> {
    ...
    // Compute x₀y₀ - ix₁y₂ - ix₂y₁ using Karatsuba for complex numbers multiplication
    ...
    // Compute x₀y₁ + x₁y₀ - ix₂y₂ using Karatsuba for complex numbers multiplication
    ...
    // Compute x₀y₂ + x₁y₁ + x₂y₀ using Karatsuba for complex numbers multiplication
